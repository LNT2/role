...
switch ($operation) {
    ...
    printf("Current character set: %s\n", mysqli_character_set_name($link));
			...
			$sql = "UPDATE post SET text='$_POST[text]',menu_id =$_POST[menu_id]  where id = $_POST[id_d]";
			echo $sql;
    ...
    printf("Current character set: %s\n", mysqli_character_set_name($link));
			...
			$sql = "DELETE FROM post where id = $_POST[id_d]";
			if ($link->query($sql) === TRUE) {
...
echo "<table>";
         ...
         try {
				$link = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
				$link->query("SET NAMES UTF-8 COLLATE UTF-8 unicode_ci");
